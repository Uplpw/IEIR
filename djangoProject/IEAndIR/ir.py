...
path = path_root + "\\content_raw"  # 文档文件夹目录
path_tfidf = path_root + "\\tfidf"  # tfidf词典目录
...
def work(query):
    ...
    for query_word in seg_list:
        if str(query_word) in dict_idf:
            ...
            print("query_word Exit: ", float(dict_idf[query_word]))
        else:
    ...
    for title in files:
        with open(path_tfidf + '\\' + title[:-4] + '.pickle', 'rb') as file_tfidf:
    ...
    print("order_dict_ir:", order_dict_ir)
    print("order_dict_ir_embedding:", order_dict_ir_embedding)
