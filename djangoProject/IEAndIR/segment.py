...
def save_index_file():
    root_path = path_root + '\\new\\'
    stopwords = load_stopwords(path_root + '\\stopwords1.txt')
    # 每个term在每篇文档中出现的频率
    kw_tf_dict = {}
    # 每个term出现在不同文档中的文档个数
    kw_df_dict = {}
...
def traverse_segment():
    ...
    if not os.path.exists(root_path):
        ...
        for root, dirs, files in os.walk(root_path):
            for file in files:
                with open(root + '\\' + file, 'r', encoding='utf-8') as fin:
                    words = segment(fin.read(), stopwords)
                    with open(result_path + file, 'a', encoding='utf-8') as fout:
                        for word in words:
                            fout.write(word + ' ')
                        fout.write('\n')
